...
{capture name="label" assign="label"}{sugar_translate label='LBL_SUBJECT_TYPE' module='Cases'}{/capture}
{$label|strip_semicolon}:
<span class="required">*</span>
</td>
{counter name="fieldsUsed"}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.subject_type_c.name}" 
id="{$fields.subject_type_c.name}" 
title=''       
>
{if isset($fields.subject_type_c.value) && $fields.subject_type_c.value != ''}
{html_options options=$fields.subject_type_c.options selected=$fields.subject_type_c.value}
{else}
{html_options options=$fields.subject_type_c.options selected=$fields.subject_type_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.subject_type_c.options }
{capture name="field_val"}{$fields.subject_type_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.subject_type_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.subject_type_c.name}" 
id="{$fields.subject_type_c.name}" 
title=''          
>
{if isset($fields.subject_type_c.value) && $fields.subject_type_c.value != ''}
...
<input
id="{$fields.subject_type_c.name}-input"
name="{$fields.subject_type_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.subject_type_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.subject_type_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.subject_type_c.name}-input', '{$fields.subject_type_c.name}');sync_{$fields.subject_type_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.subject_type_c.name}{literal}");
if (typeof select_defaults =="undefined")
select_defaults = [];
select_defaults[selectElem.id] = {key:selectElem.value,text:''};
//get default
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.subject_type_c.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.subject_type_c.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.subject_type_c.name}');
{literal}
function SyncToHidden(selectme){
var selectElem = document.getElementById("{/literal}{$fields.subject_type_c.name}{literal}");
var doSimulateChange = false;
...
//global variable 
sync_{/literal}{$fields.subject_type_c.name}{literal} = function(){
SyncToHidden();
}
function syncFromHiddenToWidget(){
var selectElem = document.getElementById("{/literal}{$fields.subject_type_c.name}{literal}");
...
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.simulate('keyup');
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.subject_type_c.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.subject_type_c.name}{literal}", syncFromHiddenToWidget);
{/literal}
SUGAR.AutoComplete.{$ac_key}.minQLen = 0;
SUGAR.AutoComplete.{$ac_key}.queryDelay = 0;
SUGAR.AutoComplete.{$ac_key}.numOptions = {$field_options|@count};
...
{sugar_translate label='LBL_PANEL_ASSIGNMENT' module='Cases'}
<script>
...
addToValidate('EditView', 'status', 'enum', false,'{/literal}{sugar_translate label='LBL_STATUS' module='Cases' for_js=true}{literal}' );
...
addToValidate('EditView', 'subject_type_c', 'enum', true,'{/literal}{sugar_translate label='LBL_SUBJECT_TYPE' module='Cases' for_js=true}{literal}' );
